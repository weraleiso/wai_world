...
void WAIOARepSequencer::cb_sub_rep_energy(const geometry_msgs::TwistStampedPtr& msg)
{
    m_vc3_rep_seq_energy.setX(msg->twist.linear.x); // Potential E
    m_vc3_rep_seq_energy.setY(msg->twist.linear.y); // Kinetic E
    m_vc3_rep_seq_energy.setZ(msg->twist.linear.z); // Total E
}
...
void WAIOARepSequencer::cb_sub_clk_gazebo(const rosgraph_msgs::ClockPtr& msg)
{
       ...
       m_f_rep_seq_elapsed!=m_f_rep_seq_duration)
    {
        ...
        sprintf(m_c_state_info,"<table border=\"1\">"
                               ...
                               "<tr> <td bgcolor=\"lightgrey\">Ene[J]</td><td>Pot</td><td>%3.3f</td> <td>Kin</td><td>%3.3f</td> <td>Tot</td><td>%3.3f</td> </tr>"
                               "</table><br><br>",
                ...
                m_vc3_rep_seq_energy.getX(),
                m_vc3_rep_seq_energy.getY(),
                m_vc3_rep_seq_energy.getZ());
                                    ...
                                    std::vector<double> vec_d_rep_seq_force)
{
    ...
    m_pub_hea_reset=m_hdl_node->advertise<std_msgs::Header>("/wai_world/"+m_s_rep_seq_name+"/state_reset",1); // Publish state info in RViz, but selectively on reset ("enable" in frame_id)
    m_sub_tst_rep_energy=m_hdl_node->subscribe("/wai_world/"+m_s_rep_seq_name+"/state_energy",1,&WAIOARepSequencer::cb_sub_rep_energy,this); // Retrieve energy from Gazebo state plugin, since not accessible here.
    ResetSequencerStates();
    ...
    // Reset according REP to starting pose (first position in sequence with zero forces and twists)
...
{
    m_vc3_rep_seq_position.setZero();
    m_vc3_rep_seq_orientation.setZero();
    m_vc3_rep_seq_vel_linear.setZero();
    m_vc3_rep_seq_vel_angular.setZero();
    m_vc3_rep_seq_force.setZero();
    m_vc3_rep_seq_torque.setZero();
    m_vc3_rep_seq_energy.setZero();
