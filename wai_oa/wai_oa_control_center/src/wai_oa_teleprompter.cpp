...
void WAIOATeleprompter::Initialize(ros::NodeHandle* hdl_node,
                                   ...
                                   int i_txt_delta)
{
    ...
    i_size_y=int(double(m_os_screen->height)*0.08);
    i_offset_x=int(double(m_os_screen->width-i_size_x)*0.5);
    i_offset_y=int(double(m_os_screen->height-i_size_y-45)); // Considers height of OpenCV window title bar
    ...
    */
    ...
    // Publish teleprompter text to OAPanel
    pub_hea_teleprompter_notes=m_hdl_node->advertise<std_msgs::Header>("audience_request_to_panel",1);

    // Set constant f_node_sample_frequency=20.0 to maintain correct pace of teleprompter
    tmr_teleprompter=m_hdl_node->createTimer(ros::Duration(1.0/20.0),&WAIOATeleprompter::cb_tmr_teleprompter,this,false,true);
}

void WAIOATeleprompter::UpdateModel(std::string s_text,int i_scene_current)
{
    i_counter=i_size_x; // Reset text offset counter
    s_teleprompter_text="NOTES (Scene #"+std::to_string(i_scene_current)+") |"+s_text+"|";

    // Publish to OAPanel
    std_msgs::Header msg_hea_notes;
    msg_hea_notes.stamp=ros::Time::now();
    msg_hea_notes.frame_id=s_teleprompter_text;
    pub_hea_teleprompter_notes.publish(msg_hea_notes);
}
...
void WAIOATeleprompter::UpdateView()
{
    ...
    cv::imshow("Teleprompter",mat_img_teleprompter);
    cv::moveWindow("Teleprompter",i_offset_x,i_offset_y);
    i_counter-=i_text_delta; // Shift text to the left (default=20pixel)
    cv::waitKey(1);
}
