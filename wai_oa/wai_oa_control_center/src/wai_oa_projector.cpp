...
void WAIOAProjector::cb_tmr_projector_play_video(const ros::TimerEvent& event)
{
    if(m_vcp_play_video.isOpened())
    {
        ...
        {
            m_pub_img_projector.publish(EncodeImage(m_mat_play_video_frame));
        }
    }
}
...
void WAIOAProjector::cb_tmr_projector(const ros::TimerEvent& event)
{
    ...
    if(f_tim_elapsed<m_f_transition_time)
    {
        ...
        m_pub_img_projector.publish(EncodeImage(m_mat_img_projector_transition));
                                        ...
                                        std::string s_path_image_default)
{
    ...
    m_pub_img_projector=m_hdl_node->advertise<sensor_msgs::CompressedImage>("projector/image_raw/compressed",10,true); // Modified: Added latched Publisher!
    ...
    m_tmr_projector_ooi=m_hdl_node->createTimer(ros::Duration(1.0/m_f_node_sample_frequency*20.0),&WAIOAProjector::cb_tmr_projector_ooi,this,false,true);
...
void WAIOAProjector::SetProjectionPose(float f_pose_x,float f_pose_y,float f_pose_z,float f_pose_yaw)
{
    ...
    m_tf_world_wrt_projection.setRotation(qua_world_wrt_projection*tf::Quaternion(0.5,-0.5,-0.5,0.5));
}
...
void WAIOAProjector::cb_m_sub_img_camera_robot_rgb(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    try
    {
        m_pub_img_projector.publish(EncodeImage(m_mat_img_camera_robotcam));
    }
    catch (cv_bridge::Exception& e)
    {
        ROS_DEBUG("cv_bridge exception: %s", e.what());
...
void WAIOAProjector::cb_m_sub_img_camera_livecam(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    try
    {
        m_pub_img_projector.publish(EncodeImage(m_mat_img_camera_livecam));
    }
    catch (cv_bridge::Exception& e)
    {
        ROS_DEBUG("cv_bridge exception: %s", e.what());
...
sensor_msgs::CompressedImage WAIOAProjector::EncodeImage(cv::Mat mat_input)
{
