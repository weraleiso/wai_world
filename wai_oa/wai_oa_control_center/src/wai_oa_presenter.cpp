...
void WAIOAPresenter::cb_tmr_presenter(const ros::TimerEvent& event)
{
    ...
    ((WAIRepOOI*)wai_oa_rep_ooi_hand_left)->UpdateModel(
                ...
                tf::Vector3(0.025,0.25,0.075),"HAL",col_blue_trans,"hand_left",m_s_path_resources_reps);
    ...
    ((WAIRepOOI*)wai_oa_rep_ooi_hand_right)->UpdateModel(
                ...
                tf::Vector3(0.025,0.25,0.075),m_s_hand_right_text,col_red_trans,"hand_right",m_s_path_resources_reps);
    ...
    ((WAIRepOOI*)wai_oa_rep_ooi_head)->UpdateModel(
                ...
                tf::Vector3(0.025,0.025,0.075),"Head",col_green_trans,"head",m_s_path_resources_reps,false,1.1);
    wai_oa_rep_ooi_head->UpdateView();
}
void WAIOAPresenter::cb_tmr_presenter_tf(const ros::TimerEvent& event)
{
    m_tfb_transforms->sendTransform(tf::StampedTransform(GetPresenterPose(),ros::Time::now(),"world","camera_rgbd_out_link"));
}
...
void WAIOAPresenter::cb_tmr_avatar_2d(const ros::TimerEvent& event)
{
    ...
    {
        pub_img_camera_2d.publish(EncodeImage(mat_img_avatar_2d));
    }
                                        ...
                                        )
{
    ...
    m_s_path_resources_reps=s_path_resources+"reps/";
    ...
    sub_img_camera_virtual_rgbd_rgb.shutdown();
    sub_img_camera_virtual_rgbd_depth.shutdown();
    ...
    sub_img_camera_2d=m_hdl_node->subscribe("camera_rgb_in/usb_cam/image_raw/compressed",1,&WAIOAPresenter::cb_sub_img_camera_2d,this);//,hints_camera_rgb_color);
    ...
    pub_img_camera_2d=m_hdl_node->advertise<sensor_msgs::CompressedImage>("camera_rgb_out/usb_cam/image_raw/compressed",1); // Modified
    ...
    */
    ...
    m_f_mood=0.0f;
    m_f_presence_visual=0.0f;
    m_b_enable_body_interaction=false;
    m_b_enable_avatar=false;
    m_b_enable_kmf=false;

    // HAND REPs Initialization
    wai_oa_rep_ooi_hand_left=WAISymbol::create_representative("OOI");
    wai_oa_rep_ooi_hand_left->Initialize(m_hdl_node,s_path_nodename,"wai_oa_rep_ooi_hand_left","world");
    ((WAIRepOOI*)wai_oa_rep_ooi_hand_left)->UpdateModel(
                m_tf_world_wrt_hand_left.getOrigin(),
                m_tf_world_wrt_hand_left.getRotation(),
                tf::Vector3(0.025,0.25,0.075),"HAL",col_blue_trans,"hand_left",m_s_path_resources_reps,true);

    wai_oa_rep_ooi_hand_right=WAISymbol::create_representative("OOI");
    wai_oa_rep_ooi_hand_right->Initialize(m_hdl_node,s_path_nodename,"wai_oa_rep_ooi_hand_right","world");
    ((WAIRepOOI*)wai_oa_rep_ooi_hand_right)->UpdateModel(
                m_tf_world_wrt_hand_right.getOrigin(),
                m_tf_world_wrt_hand_right.getRotation(),
                tf::Vector3(0.025,0.25,0.075),m_s_hand_right_text,col_red_trans,"hand_right",m_s_path_resources_reps,true);

    wai_oa_rep_ooi_head=WAISymbol::create_representative("OOI");
    wai_oa_rep_ooi_head->Initialize(m_hdl_node,s_path_nodename,"wai_oa_rep_ooi_head","world");
    ((WAIRepOOI*)wai_oa_rep_ooi_head)->UpdateModel(
                m_tf_world_wrt_head.getOrigin(),
                m_tf_world_wrt_head.getRotation(),
                tf::Vector3(0.025,0.025,0.075),"Head",col_oa_shiny,"head",m_s_path_resources_reps,true,1.1);
    ...
    m_tmr_presenter_tf=m_hdl_node->createTimer(ros::Duration(1.0/m_f_node_sample_frequency*20.0),&WAIOAPresenter::cb_tmr_presenter_tf,this,false,true);
    m_tmr_avatar_2d=m_hdl_node->createTimer(ros::Duration(1.0/m_f_node_sample_frequency*2.0),&WAIOAPresenter::cb_tmr_avatar_2d,this,false,false);
}

void WAIOAPresenter::UpdateModel(float f_mood,float f_presence_visual,bool b_enable_kmf)
{
    m_f_mood=f_mood;
    m_f_presence_visual=f_presence_visual;
    m_b_enable_kmf=b_enable_kmf;

    if(m_f_presence_visual==0.0f) // NO PRESENCE
    {
    ...
    else if(m_f_presence_visual==0.25f) // 2D-AVATAR
    {
        // RESET PRESENTER INTERACTIONS
    ...
    else if(m_f_presence_visual==0.5f) // 2D-CAMERA
    {
        // RESET PRESENTER INTERACTIONS
        m_b_enable_body_interaction=true;
        m_b_enable_avatar=false;
    ...
    else if(m_f_presence_visual==0.75f) // 3D-AVATAR (With NITE OpenNI library and movable avatar)
    {
        // DISABLE 2D-Mode
        sub_img_camera_2d.shutdown();
        DisableCameraRGB();
    ...
    else if(m_f_presence_visual==1.0f) // 3D-POINTCLOUD (from Kinect-like sensor)
    {
        // DISABLE 2D-Mode
        sub_img_camera_2d.shutdown();
        DisableCameraRGB();
...
float WAIOAPresenter::GetMood()
{
    return m_f_mood;
}
void WAIOAPresenter::SetMood(float f_mood)
{
    m_f_mood=f_mood;
}

float WAIOAPresenter::GetPresenceVisual()
{
    return m_f_presence_visual;
}
void WAIOAPresenter::SetPresenceVisual(float f_presence_visual)
{
    m_f_presence_visual=f_presence_visual;
}
...
void WAIOAPresenter::SetupRespawn2D3D(std::string s_rep_respawn_2d3d)
{
    ((WAIRepOOI*)wai_oa_rep_ooi_hand_right)->UpdateModel(
                tf::Vector3(0.0,0.0,0.0),
                tf::Quaternion(0.0,0.0,-1.0,0.0),
                tf::Vector3(0.025,0.25,0.075),s_rep_respawn_2d3d,col_red_trans,s_rep_respawn_2d3d,m_s_path_resources_reps,true);
    wai_oa_rep_ooi_hand_right->UpdateView();
}
...
tf::Transform WAIOAPresenter::GetPresenterPose()
{
...
geometry_msgs::PoseStamped WAIOAPresenter::GetPresenterPoseStamped()
{
    geometry_msgs::PoseStamped pst_pose;
    m_tf_presenter=m_tf_world_wrt_presenter_camera;
    pst_pose.header.stamp=ros::Time::now();
    pst_pose.pose.position.x=m_tf_presenter.getOrigin().getX();
    pst_pose.pose.position.y=m_tf_presenter.getOrigin().getY();
    pst_pose.pose.position.z=m_tf_presenter.getOrigin().getZ();
    pst_pose.pose.orientation.w=m_tf_presenter.getRotation().getW();
    pst_pose.pose.orientation.x=m_tf_presenter.getRotation().getX();
    pst_pose.pose.orientation.y=m_tf_presenter.getRotation().getY();
    pst_pose.pose.orientation.z=m_tf_presenter.getRotation().getZ();
    return pst_pose;
}
...
void WAIOAPresenter::ProcessBodyInteraction()
{
    ...
    if(m_f_presence_visual==1.0f) // Only process in 3D-POINTCLOUD presence mode
    {
...
///////////////////////////////////////////////////
void WAIOAPresenter::cb_sub_img_camera_2d(const sensor_msgs::CompressedImageConstPtr& msg)
{
    // Check for proper visual presence mode,
    // only process in 2D Webcam mode!
    if(m_f_presence_visual!=0.5f) return;

    pub_img_camera_2d.publish(msg);
}

void WAIOAPresenter::DisableCameraRGB()
{
    cv::Mat mat_img_empty(1,1,CV_8UC3,cv::Scalar(0,0,0));
    pub_img_camera_2d.publish(EncodeImage(mat_img_empty));
}
...
{
    // Check for proper visual presence mode,
    // only process in 3D-POINTCLOUD mode:
    if(m_f_presence_visual!=1.0f) return;
...
sensor_msgs::CompressedImage WAIOAPresenter::EncodeImage(cv::Mat mat_input)
{
