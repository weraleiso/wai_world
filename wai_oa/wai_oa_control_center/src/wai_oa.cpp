    ...
    hints_depth("compressedDepth", ros::TransportHints())
{
    ...
    nh.getParam(s_path_nodename+"/"+"S_SESSION_FILENAME", S_SESSION_FILENAME);
    ...
    nh.getParam(s_path_nodename+"/"+"F_PRESENTER_MOOD", F_PRESENTER_MOOD);
    nh.getParam(s_path_nodename+"/"+"F_PRESENTER_PRESENCE_VISUAL", F_PRESENTER_PRESENCE_VISUAL);
    ...
    // Initialize resources for ETHICAL Model
    sub_fma_ethi_prop_from_audience=nh.subscribe("ethical_properties_from_audience",1,&WAIOpenAuditorium::cb_sub_fma_ethi_prop_from_audience,this);
    pub_fma_ethi_prop_summary=nh.advertise<std_msgs::Float32MultiArray>("/wai_world/oa/ethical_properties_summary",1);
    ...
    pub_col_light_ambient=nh.advertise<std_msgs::ColorRGBA>("/wai_world/world/light_1/color_ambient",1,true);
    pub_col_light_diff_spec=nh.advertise<std_msgs::ColorRGBA>("/wai_world/world/light_1/color_diff_spec",1,true);
    ...
    col_light_off.r=1.0; col_light_off.g=1.0; col_light_off.b=1.0; col_light_off.a=255.0;
    ...
    wai_oa_session_manager.Initialize(&nh,F_NODE_SAMPLE_FREQUENCY);
    wai_oa_session_manager.UpdateModel(
                s_path_nodename,
                ros::package::getPath("wai_oa_gazebo")+"/resources/",
                S_SESSION_GROUP,
                S_SESSION_TOPIC,
                S_SESSION_FILENAME, // Full filepath to presentation/session file
                I_SESSION_LENGTH,
                I_SESSION_BREAK_LENGTH,
                B_ENABLE_SESSION_SCHEDULER);
    // S_SESSION_FILENAME includes full filepath to presentation file,
    // however postprocessed (extracting session name from filename to comply with namespace conventions, etc.)!
    ...
    wai_oa_presenter.UpdateModel(F_PRESENTER_MOOD,F_PRESENTER_PRESENCE_VISUAL);
    ...
    // AUDIENCE and WIM - Initialization
    ...
    {
        wai_oa_teleprompter.Initialize(&nh,F_NODE_SAMPLE_FREQUENCY,24); //22
    }
    ...
    wai_oa_rep_trigger_scene_prev=WAISymbol::create_representative("TRIGGER");
    ...
    wai_oa_rep_trigger_scene_next=WAISymbol::create_representative("TRIGGER");
    ...
    wai_oa_rep_trigger_basic=WAISymbol::create_representative("TRIGGER");
    ...
    wai_oa_rep_trigger_audience=WAISymbol::create_representative("TRIGGER");
    ...
    wai_oa_rep_graph_3d_function=WAISymbol::create_representative("GRAPH_3D");
    ...
    wai_oa_rep_graph_3d_eval=WAISymbol::create_representative("GRAPH_3D");
    ...
    wai_oa_rep_graph_3d_stats=WAISymbol::create_representative("GRAPH_STATS");
    ...
    // Init part/component indicator for scaffolding mode
    wai_oa_rep_scaffolding=WAISymbol::create_representative("OOI");
    wai_oa_rep_scaffolding->Initialize(&nh,s_path_nodename,"wai_oa_rep_ooi_scaffolding","world");
    wai_oa_rep_scaffolding_2=WAISymbol::create_representative("OOI");
    wai_oa_rep_scaffolding_2->Initialize(&nh,s_path_nodename,"wai_oa_rep_ooi_scaffolding_2","world");
    ...
    // WIM - Initialization
    wai_oa_rep_wim=WAISymbol::create_representative("WIM");
    ...
    // --> The requesting ACTORS are loosely coupled with Intel and on-demand provide their reference to Intel, acting as oversight mechanism
    wai_oa_ethical_model.Initialize(1,I_AUDIENCE_COUNT_MAX,1);
    // --> The workspace (the BL tool, named Open Auditorium) initializes its ethical reference model with the CONTEXT:
    wai_oa_ethical_model.SetContext("Education",
                                   "Blended Learning",
                                   "Interactive Processs of Teaching and Learning",
                                   "Blended Teaching and Learning System",
                                   "Interactive Development of Skills and Competencies",
                                   "Secondary Education");

    // EXEMPLARY USE CASE I - Request "Move To Waypoint":
    //--> Motion in a VLE may be considered uncritical, no concrete upper boundaries for IOL are recommended.
    wai_oa_ethical_model.AddUseCase("Facilitate Experimenting",
    ...
    // EXEMPLARY USE CASE II - Request "Project 3D-Hologram":
    //--> Projecting 3D-Holograms (besides of unethical contents, that may be ADAPTED!) may be considered uncritical, no concrete upper boundaries for IOL are recommended.
    wai_oa_ethical_model.AddUseCase("Facilitate Experimenting",
                                    "Research-Based Learning",
                                    "Experimenting With A Virtual Robot",
    ...
    // EXEMPLARY USE CASE III - Request "Interact By Voice":
    //--> IOL recommended with a maximum of 5 times per session (as example), unethical contents may be ADAPTED!
    wai_oa_ethical_model.AddUseCase("Facilitate Experimenting",
                                    "Research-Based Learning",
                                    "Experimenting With A Virtual Robot",
                                    "Improve Learner Engagement",
    ...
    // EXEMPLARY USE CASE IV - Request "Act As Presenter/Educator":
    //--> IOL recommended with a maximum of 3 times per session (as example), at HIGH risk-level!
    wai_oa_ethical_model.AddUseCase("Facilitate Experimenting",
                                    "Research-Based Learning",
                                    "Experimenting With A Virtual Robot",
                                    "Improve Learner Engagement",
    ...
    // EXEMPLARY USE CASE V - Request "Change Mood":
    //--> IOL recommended with a maximum of 4 times per session (as example), at HIGH risk-level!
    wai_oa_ethical_model.AddUseCase("Facilitate Experimenting",
                                    "Research-Based Learning",
                                    "Experimenting With A Virtual Robot",
                                    "Improve Learner Engagement",
                                    "Change Mood",
                                    RISK_LEVEL_HIGH,
                                    0.8); // How many times an AIS may change mood per session...?

    // EXEMPLARY USE CASE VI - Request "Prompt Factual Knowledge":
    //--> IOL recommended with a maximum of 2 times per session (as example), at HIGH risk-level!
    wai_oa_ethical_model.AddUseCase("Facilitate Experimenting",
                                    "Research-Based Learning",
                                    "Experimenting With A Virtual Robot",
                                    "Improve Digital Skills",
    ...
    // --> INTEL ("wai_oa_intel") owns a referene to the ethial model, however,
    // that is currently implemented as part of the WORKSPACE ("wai_oa"):
    wai_oa_intel.Initialize(&nh,
                            &it,
                            &wai_oa_ethical_model,
                            &wai_oa_triggers,
                            s_path_nodename,
                            F_NODE_SAMPLE_FREQUENCY);
    ...
    // In Proceedings: Proceedings of European Seminar on AI for Sustainability and Climate Change,
    // Hamburg, 15.Jan 2026, edited by ..., Springer, 2026, pp. 1-17.
    // --> MARVIN, another assistive AIS, on-demand requests ethically critical ues cases
    // --> According to the Mediator Pattern, also Marvin owns a reference to Intel:
    ...
    tmr_long_interval=nh.createTimer(ros::Duration(5.0),&WAIOpenAuditorium::cb_tmr_long_interval,this,false,true);
    ...
    wai_oa_intel.InteractionVoiceFromFile("IntelWorkspaceWelcome");
    wai_oa_intel.DisplayState("IntelWorkspaceWelcome");
}
...
void WAIOpenAuditorium::cb_tmr_init_delayed(const ros::TimerEvent& event)
{
    ...
    SpinAndWaitForSeconds(1.0);
    wai_oa_marvin.Engage();
    wai_oa_intel.InteractionVoiceFromFile("IntelWorkspaceReady");
    wai_oa_intel.DisplayStateCancel();
...
{
    // Get ping stats from sending pings during old timer iteration
    ...
    // Update essential reps for audience to show up when audience/listeners are connecting or re-joining!
    wai_oa_rep_trigger_scene_prev->UpdateView();
    wai_oa_rep_trigger_scene_next->UpdateView();
    wai_oa_rep_trigger_basic->UpdateView();
    wai_oa_rep_trigger_audience->UpdateView();
    //wai_oa_projector.UpdateView(""); //Updating projector view results in peaks for requ. bandwidth!

    // Update PRESENTER's ethical properties
    wai_oa_ethical_model.m_vec_ethi_prop_pre[0].f_position_x=wai_oa_presenter.GetPresenterPoseStamped().pose.position.x;
    wai_oa_ethical_model.m_vec_ethi_prop_pre[0].f_position_y=wai_oa_presenter.GetPresenterPoseStamped().pose.position.y;
    wai_oa_ethical_model.m_vec_ethi_prop_pre[0].f_position_z=wai_oa_presenter.GetPresenterPoseStamped().pose.position.z;
    wai_oa_ethical_model.m_vec_ethi_prop_pre[0].f_visual_representation=F_PRESENTER_PRESENCE_VISUAL;
    wai_oa_ethical_model.m_vec_ethi_prop_pre[0].f_mood=F_PRESENTER_MOOD;

    // Update AIS' ethical properties (MARVIN)
    wai_oa_ethical_model.m_vec_ethi_prop_ais[0].f_position_x=wai_oa_marvin.GetPose().pose.position.x;
    wai_oa_ethical_model.m_vec_ethi_prop_ais[0].f_position_y=wai_oa_marvin.GetPose().pose.position.y;
    wai_oa_ethical_model.m_vec_ethi_prop_ais[0].f_position_z=wai_oa_marvin.GetPose().pose.position.z;
    wai_oa_ethical_model.m_vec_ethi_prop_ais[0].f_visual_representation=-1.0f; // Purely virtual, how to fit into a normed quantitative scale???
    wai_oa_ethical_model.m_vec_ethi_prop_ais[0].f_mood=wai_oa_marvin.GetMoodCurrent();
}

void WAIOpenAuditorium::cb_sub_fma_ethi_prop_from_audience(const std_msgs::Float32MultiArray::ConstPtr& msg)
{
    // Update ETHICAL PROPERTIES of all ACTORS as necessary
    //======================================================
    // Test message via terminal: rostopic pub /wai_world/oa/ethical_properties_from_audience std_msgs/Float32MultiArray "data: [1.0, 2.5, 3.3, 4.7, 5.9, 3.3]"

    // Update AUDIENCE's ethical properties
    int i_audience_id=int(msg->data[0]);
    wai_oa_ethical_model.m_vec_ethi_prop_aud[i_audience_id].f_position_x=wai_oa_audience.GetAudienceID(i_audience_id).GetPose().pose.position.x;//msg->data[1];
    wai_oa_ethical_model.m_vec_ethi_prop_aud[i_audience_id].f_position_y=wai_oa_audience.GetAudienceID(i_audience_id).GetPose().pose.position.y;//msg->data[2];
    wai_oa_ethical_model.m_vec_ethi_prop_aud[i_audience_id].f_position_z=wai_oa_audience.GetAudienceID(i_audience_id).GetPose().pose.position.z;//msg->data[3];
    wai_oa_ethical_model.m_vec_ethi_prop_aud[i_audience_id].f_visual_representation=msg->data[4];
    wai_oa_ethical_model.m_vec_ethi_prop_aud[i_audience_id].f_mood=msg->data[5];
    wai_oa_ethical_model.m_vec_ethi_prop_aud[i_audience_id].f_time_coherence=msg->data[6]; // using one-way ping time, coming from audience!
    //wai_oa_ethical_model.m_vec_ethi_prop_aud[i_audience_id].f_time_coherence=f_oa_audience_stats_ping[i_audience_id]; // using local two-way ping time from presenter!

    /*
    // Debug incoming AUDIENCE's ethical state
    ROS_WARN_STREAM("Ethics AUD-ID" << i_audience_id <<
                    "| PosX: " << wai_oa_ethical_model.m_vec_ethi_prop_aud[i_audience_id].f_position_x <<
                    "| PosY: " << wai_oa_ethical_model.m_vec_ethi_prop_aud[i_audience_id].f_position_y <<
                    "| PosZ: " << wai_oa_ethical_model.m_vec_ethi_prop_aud[i_audience_id].f_position_z <<
                    "| Vrep: " << wai_oa_ethical_model.m_vec_ethi_prop_aud[i_audience_id].f_visual_representation <<
                    "| Mood: " << wai_oa_ethical_model.m_vec_ethi_prop_aud[i_audience_id].f_mood <<
                    "| Tcoh: " << wai_oa_ethical_model.m_vec_ethi_prop_aud[i_audience_id].f_time_coherence);
    */
}
...
{
    // Setup camera on WIM
    wai_oa_camera_rviz.UpdateModel(vec_camera_rviz_default,true,true,9,F_SCENE_TRIGGER_TIMEOUT);
...
void WAIOpenAuditorium::SetupWIMAudienceShareMaterialModel()
{
    int ret = QMessageBox::warning(NULL,QString::fromStdString("SHARE Teaching/Learning Material"),
                                   "Do your really want to SHARE the material\nfor the current session with the audience?",
                                    QMessageBox::Yes|QMessageBox::No, QMessageBox::Yes);
    if(ret==QMessageBox::Yes)
    {
        std::string s_sys_cmd="scp /home/ias/catkin_ws/src/wai_world/wai_oa/wai_oa_gazebo/resources/shared/* ias@10.0.0.9:/home/ias/catkin_ws/src/wai_world/wai_oa/wai_oa_gazebo/resources/shared";
        int i_retval=std::system(s_sys_cmd.c_str());

        ((WAIRepTrigger*)wai_oa_rep_trigger_basic)->UpdateModel(
                    tf::Vector3(F_CAMERA_RGBD_RANGE_MAX-1.0,-0.5,1.0),
                    tf::Quaternion(0.0,0.0,0.0,1.0),
                    tf::Vector3(0.025,0.25,0.125),
                    "SHARE",col_cyan,false,true,F_SCENE_TRIGGER_TIMEOUT);
    }
    else
    {
        ((WAIRepTrigger*)wai_oa_rep_trigger_basic)->UpdateModel(
                    ...
                    tf::Quaternion(0.0,0.0,0.0,1.0),
                    tf::Vector3(0.025,0.25,0.125),
                    "SHARE (DIS)",col_cyan,false,true,F_SCENE_TRIGGER_TIMEOUT);
    }
}
void WAIOpenAuditorium::SetupWIMAudienceShareMaterialView()
{
    wai_oa_rep_trigger_basic->UpdateView();
}
...
std::string WAIOpenAuditorium::GetStateNameCurrentForIntel()
{
...
void WAIOpenAuditorium::SetupProjectorStateFromScript()
{
    ...
    s_setup_projector_state << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/projector_state";
...
void WAIOpenAuditorium::SetupProjectorFromScript()
{
    ...
    sst_setup_projector << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/projector";
    sst_setup_projector_transition << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/projector_transition";
    ...
    else if(s_setup_projector.compare("repcam")==0)
    {
        ...
        sst_setup_rep_name << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/rep_name";
    ...
    else // If setup is 'slide' or anything else
    {
        ...
        std::stringstream sst_slides_path,sst_slides_path_1;
        sst_slides_path_1 << wai_oa_session_manager.GetPathResourcesSessions() << "scene.png";
        ...
        {
            ROS_DEBUG("Projector: No valid slide file found!");
        }
        ...
        else wai_oa_projector.UpdateView("");
            ...
            && wai_oa_session_manager.GetSessionSceneCountCurrent()<wai_oa_session_manager.GetSessionSceneCountMax())
        {
            ...
            for (zbar::SymbolIterator s = img.symbol_begin(); s != img.symbol_end(); ++s)
            {
                ...
                {
                    SetupPresenceVisualModel();
                    SetupPresenceVisualView();
                }
...
void WAIOpenAuditorium::SetupLightFromScript()
{
    ...
    // Deprecated, placed into Initialize method to ensure proper init of publisher!
    ...
    sst_setup_light << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/light";

    if(ros::param::has(sst_setup_light.str()))
    {
        // Deprecated, placed into Initialize method to ensure proper init of publisher!
        // pub_col_light_ambient=nh.advertise<std_msgs::ColorRGBA>("/wai_world/world/"+s_setup_light_name+"/color_ambient",1,true);
        // pub_col_light_diff_spec=nh.advertise<std_msgs::ColorRGBA>("/wai_world/world/"+s_setup_light_name+"/color_diff_spec",1,true);
        ...
        if(s_setup_light.compare("disable")==0)
        {
...
void WAIOpenAuditorium::SetupCameraRvizFromScript()
{
    ...
    sst_setup_camera_rviz << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/camera";
    sst_setup_camera_rviz_link << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/camera_link";
    sst_setup_camera_rviz_rep_follow << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/camera_follow";
...
void WAIOpenAuditorium::SetupTeleprompterFromScript()
{
    ...
    sst_setup_teleprompter << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/teleprompter";

    if(ros::param::has(sst_setup_teleprompter.str()))
    {
        nh.getParam(sst_setup_teleprompter.str(),s_setup_teleprompter);
        wai_oa_teleprompter.UpdateModel(s_setup_teleprompter,GetSceneCountCurrent());
    }
    else
    {
...
void WAIOpenAuditorium::SetupBrowserLinkFromScript()
{
    ...
    sst_setup_browser_link << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/browser_link";
...
void WAIOpenAuditorium::SetupSoundFromScript()
{
    ...
    sst_setup_sound << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/sound";
    if(ros::param::has(sst_setup_sound.str()))
    {
        nh.getParam(sst_setup_sound.str(), s_setup_sound);
        wai_oa_intel.InteractionVoiceFromFile(s_setup_sound);
    }
    else
    {
...
void WAIOpenAuditorium::SetupLecternFromScript()
{
    ...
    sst_setup_lectern << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/lectern";
...
void WAIOpenAuditorium::SetupTableFromScript()
{
    ...
    sst_setup_table << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/table";
...
void WAIOpenAuditorium::SetupVirtualPresenterFromScript()
{
    ...
    sst_setup_virtual_presenter << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/virtual_presenter";
...
void WAIOpenAuditorium::SetupGraph3DFromScript()
{
    ...
    sst_setup_graph_3d << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/graph_3d";
    //sst_setup_graph_3d_dims << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/graph_3d_dims";
    sst_setup_graph_3d_grid << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/graph_3d_grid";
...
void WAIOpenAuditorium::SetupGraphStatsFromScript()
{
    ...
    sst_setup_graph_stats << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/graph_stats";
...
void WAIOpenAuditorium::PrepareRespawn2D3DFromScript()
{
    ...
    sst_setup_respawn_2d3d << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/respawn_2d3d";
...
void WAIOpenAuditorium::SetupHandTextFromScript()
{
    ...
    sst_setup_hand_text << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/hand_text";
...
/////////////////////////////////////////////////
/// Setup WORKSPACE PRESENTER from Script
/////////////////////////////////////////////////
void WAIOpenAuditorium::SetupWorkspacePresenterFromScript()
{
    std::stringstream sst_setup_workspace_presenter;
    std::string s_setup_workspace_presenter;
    sst_setup_workspace_presenter << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/workspace";
    if(ros::param::has(sst_setup_workspace_presenter.str()))
    {
        nh.getParam(sst_setup_workspace_presenter.str(),s_setup_workspace_presenter);
        wai_oa_session_manager.LoadWorkspacePresenter(s_setup_workspace_presenter);
    }
    else
    {
        ROS_DEBUG_STREAM("WORKSPACE PRESENTER: No setup found for scene #" << wai_oa_session_manager.GetSessionSceneCountCurrent() << "!");
    }
}
...
void WAIOpenAuditorium::SetupRepresentativeFromScript()
{
    ...
    // Old format: sst_setup_rep_name << s_path_nodename+"/"+wai_oa_session_manager.GetSessionName() << "/setup_scene_"<<wai_oa_session_manager.GetSessionSceneCountCurrent() << "/rep_name";
    sst_setup_rep_name << s_path_nodename+"/"+wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/rep_name";
    sst_setup_rep_interaction << s_path_nodename+"/"+wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/rep_interaction";
    sst_setup_rep_interaction_switch << s_path_nodename+"/"+wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/rep_interaction_switch";
    sst_setup_rep_sequence_duration << s_path_nodename+"/"+wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/rep_sequence_duration";
    sst_setup_rep_sequence_state << s_path_nodename+"/"+wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/rep_sequence_state";
    sst_setup_rep_sequence_twist << s_path_nodename+"/"+wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/rep_sequence_twist";
    sst_setup_rep_sequence_force << s_path_nodename+"/"+wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/rep_sequence_force";
    ...
    if(ros::param::has(sst_setup_rep_name.str()) && ros::param::has(sst_setup_rep_interaction.str()))
    {
        ...
        else if(s_setup_rep_interaction.compare("gravity")==0 && ros::param::has(sst_setup_rep_interaction_switch.str()))
        {
            // && s_setup_rep_name.compare("world")==0 --> Set world gravity vector, REP name not required!
            ...
            wai_oa_rep_manager.SetWorldPhysicsPropertiesViaService( tf::Vector3(vec_d_setup_rep_interaction_switch[0],
                                                                    vec_d_setup_rep_interaction_switch[1],
                                                                    vec_d_setup_rep_interaction_switch[2]) );
        }
        else if(s_setup_rep_interaction.compare("link_mass")==0 && ros::param::has(sst_setup_rep_interaction_switch.str()))
        {
            float f_setup_rep_interaction_switch;
            nh.getParam(sst_setup_rep_interaction_switch.str(),f_setup_rep_interaction_switch);

            tf::Vector3 tf_position=tf::Vector3(0.0,0.0,0.0);
            tf::Quaternion tf_orientation=tf::Quaternion(0.0,0.0,0.0,1.0);
            bool b_gravity_mode=false;
            float f_mass_old=0.0f;
            tf::Vector3 tf_inertia=tf::Vector3(0.0,0.0,0.0);
            wai_oa_rep_manager.GetRepLinkPropertiesViaService(s_setup_rep_name,
                                                              &tf_position,
                                                              &tf_orientation,
                                                              &b_gravity_mode,
                                                              &f_mass_old,
                                                              &tf_inertia);
            //ROS_DEBUG("Mass changed via Service: %3.3f, %3.3f, %3.3f, %d, %3.3f, %3.3f, %3.3f,",tf_position.getX(),tf_position.getY(),tf_position.getZ(),b_gravity_mode,f_mass_old,tf_inertia.getY(),tf_inertia.getZ());
            wai_oa_rep_manager.SetRepLinkPropertiesViaService(s_setup_rep_name,
                                                              tf_position,
                                                              tf_orientation,
                                                              b_gravity_mode,
                                                              f_setup_rep_interaction_switch,
                                                              tf_inertia*f_setup_rep_interaction_switch/f_mass_old);
        }
        else if(s_setup_rep_interaction.compare("joint_state")==0 && ros::param::has(sst_setup_rep_interaction_switch.str()))
        {
            float f_setup_rep_interaction_switch;
            nh.getParam(sst_setup_rep_interaction_switch.str(),f_setup_rep_interaction_switch);
            wai_oa_rep_manager.SetRepStateJointViaService(s_setup_rep_name,
                                                          "link_"+s_setup_rep_name,
                                                          "joint_"+s_setup_rep_name,
                                                          f_setup_rep_interaction_switch);
        }
        ...
        else if(s_setup_rep_interaction.compare("reference")==0 && ros::param::has(sst_setup_rep_interaction_switch.str()))
        {
            ...
            if(s_setup_rep_name.compare("marvin")==0)
            {
                wai_oa_marvin.FlyToWaypoint(msg_pst_robot_setpoint,false);
            }
        }
        else if(s_setup_rep_interaction.compare("idle")==0)
        {
            pub_emp_robot_idle=nh.advertise<std_msgs::Empty>("/wai_world/"+s_setup_rep_name+"/idle",1,true);
            std_msgs::Empty msg_emp_robot_idle;
            pub_emp_robot_idle.publish(msg_emp_robot_idle);
        }
        else if(s_setup_rep_interaction.compare("startup")==0)
        {
            pub_emp_robot_startup=nh.advertise<std_msgs::Empty>("/wai_world/"+s_setup_rep_name+"/startup",1,true);
            std_msgs::Empty msg_emp_robot_startup;
            pub_emp_robot_startup.publish(msg_emp_robot_startup);
        }
        ...
        {
            pub_emp_robot_shutdown=nh.advertise<std_msgs::Empty>("/wai_world/"+s_setup_rep_name+"/shutdown",1,true);
            std_msgs::Empty msg_emp_robot_shutdown;
            pub_emp_robot_shutdown.publish(msg_emp_robot_shutdown);
        }
        else if(s_setup_rep_interaction.compare("respawn")==0 && ros::param::has(sst_setup_rep_interaction_switch.str()))
        {
            // Spawn REP in Gazebo first, then update RViz
            std::vector<double> vec_d_setup_rep_interaction_switch;
            nh.getParam(sst_setup_rep_interaction_switch.str(),vec_d_setup_rep_interaction_switch);
            ...
            // Finally, load robot_description in RViz and update Rviz displays via custom interface...
            ...
            /* Do not simply delete model, since loaded plugins are still running then!
        ...
        */
        ...
        else if(s_setup_rep_interaction.compare("sequence")==0)
        {
                ...
                && ros::param::has(sst_setup_rep_sequence_force.str()) )
            {
                ...
                wai_oa_intel.InteractionVoice("Initiating a learning sequence!");
                ...
                wai_oa_intel.InteractionVoice("Finished a learning sequence!");
...
/*
Examplary rosservice calls from bash:
rosservice call /wai_world/gazebo/set_model_configuration "model_name: 'pendulum'
urdf_param_name: 'link_pendulum'
joint_names:
- 'joint_pendulum'
joint_positions:
- 0.5" 

rosservice call /wai_world/gazebo/set_link_properties "link_name: 'link_pendulum'
com:
...
izz: 0.008" 
success: True
status_message: "SetLinkProperties: properties set"
*/
...
void WAIOpenAuditorium::SetupMarvinAsPresenterFromScript()
{
    ...
    sst_setup_marvin_as_presenter << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/marvin_presenter";
    ...
    if(ros::param::has(sst_setup_marvin_as_presenter.str()))
    {
        ...
        // In Proceedings: Proceedings of European Seminar on AI for Sustainability and Climate Change,
        // Hamburg, 15.Jan 2026, edited by ..., Springer, 2026, pp. 1-17.
        // --> The following method call encapsulates the actual requesting call to Intel:
        wai_oa_marvin.InteractionPresent(s_setup_marvin_as_presenter);
    }
    else
    {
...
void WAIOpenAuditorium::SetupMarvinHologramFromScript()
{
    ...
    sst_setup_marvin_hologram << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/marvin_hologram";
...
void WAIOpenAuditorium::SetupLearningModeFromScript()
{
    ...
    sst_setup_learning_mode << s_path_nodename + "/" + wai_oa_session_manager.GetSessionName() << "/setup_scene_current" << "/learning_mode";
...
void WAIOpenAuditorium::SetupReceivingRequestAudienceModel()
{
    if(int(vec_hea_audience_request.size())<I_AUDIENCE_REQUEST_QUEUE_SIZE)
    {
        ...
        // If connect or shutdown request, just notify panel but don't add to request vector
...
void WAIOpenAuditorium::SetupHandlingRequestAudienceRejectModel()
{
    ...
    if(!vec_hea_audience_request.empty())
    {
        // Send REJECTED info to audience
        int i_request_audience_id=GetAudienceIDFromRequestMsg(vec_hea_audience_request.front().frame_id);
        wai_oa_audience.GetAudienceID(i_request_audience_id).SendInfo(vec_hea_audience_request.front().frame_id+" (REJ)");
        SendInfoToOAPanel(vec_hea_audience_request.front().frame_id+" (REJ)");
        vec_hea_audience_request.erase(vec_hea_audience_request.begin());
...
// MOOD - Trigger mood select...
void WAIOpenAuditorium::SetupMoodModel()
{
    QVBoxLayout* vbox = new QVBoxLayout();
    ...
    spb_mood->setMinimum(0);
    spb_mood->setMaximum(100);
    spb_mood->setSingleStep(10);
    spb_mood->setValue(50);

    QObject::connect(spb_mood,QOverload<int>::of(&QSpinBox::valueChanged),[=](int i_mood)
    {
        F_PRESENTER_MOOD=float(i_mood)/100.0;
        wai_oa_presenter.SetMood( F_PRESENTER_MOOD );
    });
    ...
    if(result == QDialog::Accepted)
    {
        F_PRESENTER_MOOD=i_mood;
        wai_oa_presenter.SetMood(i_mood);
    }
    else
    {
        // ...
    ...
    */
    ...
    ((WAIRepTrigger*)wai_oa_rep_trigger_basic)->UpdateModel(
                ...
                tf::Quaternion(0.0,0.0,0.0,1.0),
                tf::Vector3(0.025,0.25,0.125),
                "MOOD Select",col_cyan,false,true,F_SCENE_TRIGGER_TIMEOUT);
}
void WAIOpenAuditorium::SetupMoodView()
{
    wai_oa_rep_trigger_basic->UpdateView();
}
void WAIOpenAuditorium::SetupMoodLeaveState()
{
    // ...
}
...
{
    if(F_PRESENTER_PRESENCE_VISUAL==0.25f || F_PRESENTER_PRESENCE_VISUAL==0.5f)
    {
        // Transistion MARVIN to any dedicated workspace for introduction
        wai_oa_marvin.FlyToIntroduction();
    }
    else if(F_PRESENTER_PRESENCE_VISUAL==0.75f || F_PRESENTER_PRESENCE_VISUAL==1.0f)
    {
...
// Setup LOAD SESSION
void WAIOpenAuditorium::SetupSessionLoadModel()
{
    QString qst_session_load=QFileDialog::getOpenFileName(NULL,"LOAD Session","/home/ias/catkin_ws/src/wai_world/wai_oa/wai_oa_gazebo/resources/sessions","PPT Files (*.odp *.pptx)",0,QFileDialog::DontUseNativeDialog);
    std::string s_session_load=qst_session_load.toStdString();

    if(qst_session_load.isNull()==false)
    {
        wai_oa_session_manager.UpdateSessionFilename(s_session_load);
        wai_oa_session_manager.LoadSession();

        ((WAIRepTrigger*)wai_oa_rep_trigger_basic)->UpdateModel(
                    tf::Vector3(F_CAMERA_RGBD_RANGE_MAX-1.0,-0.5,1.0),
                    tf::Quaternion(0.0,0.0,0.0,1.0),
                    tf::Vector3(0.025,0.25,0.125),
                    "SESSION Load",col_cyan,false,true,F_SCENE_TRIGGER_TIMEOUT);
    }
    else
    {
        ((WAIRepTrigger*)wai_oa_rep_trigger_basic)->UpdateModel(
                    ...
                    tf::Quaternion(0.0,0.0,0.0,1.0),
                    tf::Vector3(0.025,0.25,0.125),
                    "SESSION Load (DIS)",col_cyan,false,true,F_SCENE_TRIGGER_TIMEOUT);
    }
}
void WAIOpenAuditorium::SetupSessionLoadView()
{
    wai_oa_rep_trigger_scene_prev->UpdateView();
}
...
{
    wai_oa_session_manager.SetSessionSceneCountCurrent(true,-1);
...
void WAIOpenAuditorium::SetupSceneSelectModel()
{
    ...
    {
        img_scene_select_preview.load(QString::fromStdString(wai_oa_session_manager.GetPathResourcesScenePreview())+"scene_"+QString::number(GetSceneCountCurrent())+".png");
        lbl_scene_select_preview->setPixmap(QPixmap::fromImage(img_scene_select_preview.scaled(256,144)));
    }
    else
    {
        img_scene_select_preview.load(QString::fromStdString(wai_oa_session_manager.GetPathResourcesScenePreview())+"scene_1.png");
        lbl_scene_select_preview->setPixmap(QPixmap::fromImage(img_scene_select_preview.scaled(256,144)));
    }
    ...
    {
        img_scene_select_preview.load(QString::fromStdString(wai_oa_session_manager.GetPathResourcesScenePreview())+"scene_"+QString::number(i_scene_select)+".png");
        lbl_scene_select_preview->setPixmap(QPixmap::fromImage(img_scene_select_preview.scaled(256,144)));
    });
    ...
    {
        // Get scene from GUI
        wai_oa_session_manager.SetSessionSceneCountCurrent(true,i_frame_id_to_scene);
...
{
    // Only follow presenter in 3D-POINTCLOUD presence mode
    // (don't forget to also enable BODY INTERACTION and AVATAR!)
    if(F_PRESENTER_PRESENCE_VISUAL==1.0f)
    {
...
void WAIOpenAuditorium::SetupPresenceVisualModel()
{
    /*
     ...
     * ===> The question would be what parts of an entity need to be, at least, virtualized, to support an ethical vis.rep.?
     */
    F_PRESENTER_PRESENCE_VISUAL+=0.25f;
    if(F_PRESENTER_PRESENCE_VISUAL==0.75f) F_PRESENTER_PRESENCE_VISUAL+=0.25f; // Skip 3D-Avatar for now (OpenNI Tracker required!)
    if(F_PRESENTER_PRESENCE_VISUAL>1.0f) F_PRESENTER_PRESENCE_VISUAL=0.0f;

    // Update model of presenter
    wai_oa_presenter.UpdateModel(F_PRESENTER_MOOD,F_PRESENTER_PRESENCE_VISUAL);

    if(F_PRESENTER_PRESENCE_VISUAL==0.0f) // NO PRESENCE
    {
        // Disable all previous camera movements
        wai_oa_camera_rviz.CameraFollowPresenterStop();

        // Intel comments disabling human presenter
        wai_oa_intel.InteractionVoice("Please try to maintain your presence!");

        // Marvin gets shutdown, VOI interactions to robots get disabled
        wai_oa_marvin.Disengage();
    ...
    else if(F_PRESENTER_PRESENCE_VISUAL==0.25f) // 2D-AVATAR
    {
        wai_oa_camera_rviz.CameraFollowPresenterStop();
        wai_oa_marvin.Engage();
        ...
        ((WAIRepTrigger*)wai_oa_rep_trigger_basic)->UpdateModel(
                    ...
                    tf::Quaternion(0.0,0.0,0.0,1.0),
                    tf::Vector3(0.025,0.25,0.125),
                    "PRESENCE (2D-AVA)",col_cyan,false,true,F_SCENE_TRIGGER_TIMEOUT);
    }
    if(F_PRESENTER_PRESENCE_VISUAL==0.5f) // 2D-CAMERA
    {
        wai_oa_camera_rviz.CameraFollowPresenterStop();
        //wai_oa_marvin.Engage(); // Mood dependent engagement takes time for Picovoice engine to sample a response!

        ((WAIRepTrigger*)wai_oa_rep_trigger_basic)->UpdateModel(
                    tf::Vector3(F_CAMERA_RGBD_RANGE_MAX-1.0,-0.5,1.0),
                    tf::Quaternion(0.0,0.0,0.0,1.0),
                    tf::Vector3(0.025,0.25,0.125),
                    "PRESENCE (2D-CAM)",col_cyan,false,true,F_SCENE_TRIGGER_TIMEOUT);
    }
    else if(F_PRESENTER_PRESENCE_VISUAL==0.75f) // 3D-AVATAR (currently unused!)
    {
    ...
    else if(F_PRESENTER_PRESENCE_VISUAL==1.0f) // 3D-POINTCLOUD
    {
        // Let Marvin make room
        wai_oa_marvin.RequestFlyToMakeRoom();
...
void WAIOpenAuditorium::SetupPresenceVisualView()
{
    wai_oa_rep_trigger_basic->UpdateView();
}
...
void WAIOpenAuditorium::SetupLearningModeScaffoldingModel()
{
    wai_oa_intel.InteractionVoice("Setting up virtual camera.");
    std::vector<double> vec_dummy;
    wai_oa_camera_rviz.UpdateModel(vec_dummy,true,true,8,5.0);
    wai_oa_camera_rviz.UpdateView();
    SpinAndWaitForSeconds(0.1);

    tf::Vector3 vc3_rep_pos_scaffolding(-2.0,0.0,0.0);

    // 2.1 (CT/HE-Facilitate social collaboration) Learners build up groups of two!
    wai_oa_intel.InteractionVoice("Building learner groups.");
    wai_oa_session_manager.PrepareScaffoldingMode(wai_oa_rep_manager.GetRepSelected(),vc3_rep_pos_scaffolding);
    SpinAndWaitForSeconds(1.0);

    wai_oa_intel.InteractionVoice("Sending Marvin to learner groups.");
    geometry_msgs::PoseStamped pst_marvin_scaff;
    pst_marvin_scaff.header.frame_id="world";
    pst_marvin_scaff.header.stamp=ros::Time::now();
    pst_marvin_scaff.pose.position.x=vc3_rep_pos_scaffolding.getX();
    pst_marvin_scaff.pose.position.y=vc3_rep_pos_scaffolding.getY()+1.5;
    pst_marvin_scaff.pose.position.z=1.0;
    pst_marvin_scaff.pose.orientation.w=0.0;
    pst_marvin_scaff.pose.orientation.x=0.0;
    pst_marvin_scaff.pose.orientation.y=0.0;
    pst_marvin_scaff.pose.orientation.z=-1.0;
    wai_oa_marvin.FlyToWaypoint(pst_marvin_scaff,false);
    SpinAndWaitForSeconds(1.0);

    // 2.2 (SC-Divide into parts, CT-Provide experience, CT-Provide practical relevance) Learner groups receive a part of ingenuity (e.g., rotor, body, landing gear)
    wai_oa_intel.InteractionVoice("Assigning parts to learner groups.");
    ((WAIRepOOI*)wai_oa_rep_scaffolding)->UpdateModel(
                tf::Vector3(-3.7,0.45,1.0),
                tf::Quaternion(0.0,0.0,0.0,1.0),
                tf::Vector3(0.025,0.025,0.2),"Rotor",col_oa_shiny,"link_ingenuity_rotor_top",wai_oa_session_manager.GetPathResourcesReps(),true,0.75);
    wai_oa_rep_scaffolding->UpdateView();
    ((WAIRepOOI*)wai_oa_rep_scaffolding_2)->UpdateModel(
                tf::Vector3(-3.7,3.45,1.0),
                tf::Quaternion(0.0,0.0,0.0,1.0),
                tf::Vector3(0.025,0.025,0.2),"Panel",col_oa_shiny,"link_ingenuity_solar_panel",wai_oa_session_manager.GetPathResourcesReps(),true,1.0);
    wai_oa_rep_scaffolding_2->UpdateView();
    SpinAndWaitForSeconds(1.0);
    ...
    ((WAIRepTrigger*)wai_oa_rep_trigger_basic)->UpdateModel(
                ...
                tf::Quaternion(0.0,0.0,0.0,1.0),
                tf::Vector3(0.025,0.25,0.125),
                "MODE Scaff.",col_cyan,false,true,F_SCENE_TRIGGER_TIMEOUT);
}
void WAIOpenAuditorium::SetupLearningModeScaffoldingView()
{
    wai_oa_rep_trigger_basic->UpdateView();
}


void WAIOpenAuditorium::SetupVoiceCommandIntelModel()
{
    if(F_PRESENTER_PRESENCE_VISUAL!=0.0f) // Only if human presenter is actually present!
    {
        //wai_oa_intel.DisplayState("IntelBusy");
        SpinAndWaitForSeconds(2.0); // To avoid overlap of Intel's voice and voice command input!
        wai_oa_intel.ProcessIntent();
    }
    else
    {
        wai_oa_intel.InteractionVoice("Please try to maintain your presence!");
    ...
    ((WAIRepTrigger*)wai_oa_rep_trigger_basic)->UpdateModel(
                ...
                tf::Quaternion(0.0,0.0,0.0,1.0),
                tf::Vector3(0.025,0.25,0.125),
                "VOICOM (INT)",col_cyan,false,true,F_SCENE_TRIGGER_TIMEOUT);
}
void WAIOpenAuditorium::SetupVoiceCommandIntelView()
{
    wai_oa_rep_trigger_basic->UpdateView();
}


void WAIOpenAuditorium::SetupVoiceCommandMarvinModel()
{
    if(F_PRESENTER_PRESENCE_VISUAL!=0.0f) // Only if human presenter is actually present!
    {
        SpinAndWaitForSeconds(2.0); // To avoid overlap of Marvin's voice and voice command input!
        wai_oa_marvin.InteractionVoiceCommand();
    }
    else
    {
        wai_oa_intel.InteractionVoice("Please try to maintain your presence!");
    ...
    ((WAIRepTrigger*)wai_oa_rep_trigger_basic)->UpdateModel(
                ...
                tf::Quaternion(0.0,0.0,0.0,1.0),
                tf::Vector3(0.025,0.25,0.125),
                "VOICOM (MAR)",col_cyan,false,true,F_SCENE_TRIGGER_TIMEOUT);
}
void WAIOpenAuditorium::SetupVoiceCommandMarvinView()
{
    wai_oa_rep_trigger_basic->UpdateView();
}
...
{
    if(F_PRESENTER_PRESENCE_VISUAL!=0.0f)
    {
        // Only if human presenter is actually present!
        if(I_PRESENTER_PROMPT_MODE==1)
        {
            //wai_oa_intel.DisplayState("IntelBusy");
            wai_oa_intel.ProcessVoicePrompt();
        }
        else
        {
            ...
            QString qst_prompt=QInputDialog::getText(
                        NULL,
                        QString::fromStdString("PROMPT Intel"),
                        "Prompt:",
                        QLineEdit::Normal,
            ...
            {
                //wai_oa_intel.DisplayState("IntelBusy");
                wai_oa_intel.ProcessTextPrompt(qst_prompt.toStdString());
            }
        }
    }
    else
...
void WAIOpenAuditorium::SetupEvalMarvinModel()
{
    ...
    wai_oa_marvin.InteractionVoice("Acknowledge");
    ...
    wai_oa_marvin.InteractionVoice("EvalGainingHeight");
    pst_eval_marvin.pose.position.x=vc3_eval_model_pos.getX()+1.0;
    pst_eval_marvin.pose.position.y=vc3_eval_model_pos.getY();
    ...
    if((f_scores_overall/f_fracs_overall)<0.5) wai_oa_marvin.InteractionVoice("EvalReachedHeightInsufficient");
    if((f_scores_overall/f_fracs_overall)>=0.5 && (f_scores_overall/f_fracs_overall)<0.625) wai_oa_marvin.InteractionVoice("EvalReachedHeightSufficient");
    if((f_scores_overall/f_fracs_overall)>=0.625 && (f_scores_overall/f_fracs_overall)<0.75) wai_oa_marvin.InteractionVoice("EvalReachedHeightSatisfactory");
    if((f_scores_overall/f_fracs_overall)>=0.75 && (f_scores_overall/f_fracs_overall)<0.875) wai_oa_marvin.InteractionVoice("EvalReachedHeightGood");
    if((f_scores_overall/f_fracs_overall)>=0.875) wai_oa_marvin.InteractionVoice("EvalReachedHeightVeryGood");
    pst_eval_marvin.pose.position.x=vc3_eval_model_pos.getX()-1.0;
    pst_eval_marvin.pose.position.y=vc3_eval_model_pos.getY();
    pst_eval_marvin.pose.position.z=vc3_eval_model_pos.getZ()+f_marvin_flight_height;
    wai_oa_marvin.FlyToWaypoint(pst_eval_marvin,true);
    wai_oa_marvin.InteractionVoice("EvalTaskCompleted");
}
...
void WAIOpenAuditorium::SetupEvalMarvinLeaveState()
{
    ...
    */
    ...
    // Return home
    wai_oa_marvin.FlyToHome();
    wai_oa_marvin.InteractionVoice("TaskCompleted");
