                                         ...
                                         float f_node_sample_frequency)
{
    ...
    ser_cli_gazebo_set_model_configuration=m_hdl_node->serviceClient<gazebo_msgs::SetModelConfiguration>("/wai_world/gazebo/set_model_configuration");
    ser_cli_gazebo_get_link_properties=m_hdl_node->serviceClient<gazebo_msgs::GetLinkProperties>("/wai_world/gazebo/get_link_properties");
    ser_cli_gazebo_set_link_properties=m_hdl_node->serviceClient<gazebo_msgs::SetLinkProperties>("/wai_world/gazebo/set_link_properties");
    ser_cli_gazebo_set_world_physics_properties=m_hdl_node->serviceClient<gazebo_msgs::SetPhysicsProperties>("/wai_world/gazebo/set_physics_properties");
    ...
    wai_oa_rep_detail=WAISymbol::create_representative("OOI");
...
void WAIOARepManager::SetRepStateJointViaService(std::string s_rep_name,std::string s_rep_link_name,std::string s_rep_joint_name,float f_joint_position)
{
    gazebo_msgs::SetModelConfiguration set_model_config;
    set_model_config.request.model_name=s_rep_name;
    set_model_config.request.urdf_param_name=s_rep_link_name;
    set_model_config.request.joint_names.push_back(s_rep_joint_name);
    set_model_config.request.joint_positions.push_back(f_joint_position);
    if(ser_cli_gazebo_set_model_configuration.isValid())
    {
        ser_cli_gazebo_set_model_configuration.waitForExistence();
        ser_cli_gazebo_set_model_configuration.call(set_model_config);
    }
}
void WAIOARepManager::GetRepLinkPropertiesViaService(std::string s_rep_link_name,tf::Vector3* vc3_position,tf::Quaternion* qua_orientation,bool* b_gravity_mode,float* f_mass,tf::Vector3* vc3_inertia)
{
    gazebo_msgs::GetLinkProperties get_link_props;
    get_link_props.request.link_name=s_rep_link_name;
    if(ser_cli_gazebo_get_link_properties.isValid())
    {
        ser_cli_gazebo_get_link_properties.waitForExistence();
        ser_cli_gazebo_get_link_properties.call(get_link_props);
    }
    vc3_position->setValue(get_link_props.response.com.position.x,
                            get_link_props.response.com.position.y,
                            get_link_props.response.com.position.z);
    qua_orientation->setValue(get_link_props.response.com.orientation.x,
                              get_link_props.response.com.orientation.y,
                              get_link_props.response.com.orientation.z,
                              get_link_props.response.com.orientation.w);
    *b_gravity_mode=get_link_props.response.gravity_mode;
    *f_mass=get_link_props.response.mass;
    vc3_inertia->setValue(get_link_props.response.ixx,get_link_props.response.iyy,get_link_props.response.izz);
}
void WAIOARepManager::SetRepLinkPropertiesViaService(std::string s_rep_link_name,tf::Vector3 vc3_position,tf::Quaternion qua_orientation,bool b_gravity_mode,float f_mass,tf::Vector3 vc3_inertia)
{
    gazebo_msgs::SetLinkProperties set_link_props;
    set_link_props.request.link_name=s_rep_link_name;
    set_link_props.request.com.position.x=vc3_position.getX();
    set_link_props.request.com.position.y=vc3_position.getY();
    set_link_props.request.com.position.z=vc3_position.getZ();
    set_link_props.request.com.orientation.w=qua_orientation.getW();
    set_link_props.request.com.orientation.x=qua_orientation.getX();
    set_link_props.request.com.orientation.y=qua_orientation.getY();
    set_link_props.request.com.orientation.z=qua_orientation.getZ();
    set_link_props.request.gravity_mode=b_gravity_mode;
    set_link_props.request.mass=f_mass;
    set_link_props.request.ixx=vc3_inertia.getX();
    set_link_props.request.iyy=vc3_inertia.getY();
    set_link_props.request.izz=vc3_inertia.getZ();
    if(ser_cli_gazebo_set_link_properties.isValid())
    {
        ser_cli_gazebo_set_link_properties.waitForExistence();
        ser_cli_gazebo_set_link_properties.call(set_link_props);
    }
}

void WAIOARepManager::SetWorldPhysicsPropertiesViaService(tf::Vector3 vc3_gravity,
                                                   double d_time_step,
                                                   double d_max_update_rate,
                                                   bool b_ode_auto_disable_bodies,
                                                   int i_ode_sor_pgs_precon_iters,
                                                   int i_ode_sor_pgs_iters,
                                                   double d_ode_sor_pgs_w,
                                                   double d_ode_sor_pgs_rms_error_tol,
                                                   double d_ode_contact_surface_layer,
                                                   double d_ode_contact_max_correcting_vel,
                                                   double d_ode_cfm,
                                                   double d_ode_erp,
                                                   int i_ode_max_contacts)
{
    gazebo_msgs::SetPhysicsProperties set_world_physics_props;
    set_world_physics_props.request.time_step=d_time_step;
    set_world_physics_props.request.max_update_rate=d_max_update_rate;
    set_world_physics_props.request.gravity.x=vc3_gravity.getX();
    set_world_physics_props.request.gravity.y=vc3_gravity.getY();
    set_world_physics_props.request.gravity.z=vc3_gravity.getZ();
    set_world_physics_props.request.ode_config.auto_disable_bodies=b_ode_auto_disable_bodies;
    set_world_physics_props.request.ode_config.sor_pgs_precon_iters=i_ode_sor_pgs_precon_iters;
    set_world_physics_props.request.ode_config.sor_pgs_iters=i_ode_sor_pgs_iters;
    set_world_physics_props.request.ode_config.sor_pgs_w=d_ode_sor_pgs_w;
    set_world_physics_props.request.ode_config.sor_pgs_rms_error_tol=d_ode_sor_pgs_rms_error_tol;
    set_world_physics_props.request.ode_config.contact_surface_layer=d_ode_contact_surface_layer;
    set_world_physics_props.request.ode_config.contact_max_correcting_vel=d_ode_contact_max_correcting_vel;
    set_world_physics_props.request.ode_config.cfm=d_ode_cfm;
    set_world_physics_props.request.ode_config.erp=d_ode_erp;
    set_world_physics_props.request.ode_config.max_contacts=i_ode_max_contacts;
    if(ser_cli_gazebo_set_world_physics_properties.isValid())
    {
        ser_cli_gazebo_set_world_physics_properties.waitForExistence();
        ser_cli_gazebo_set_world_physics_properties.call(set_world_physics_props);
    }
}
