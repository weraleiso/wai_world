...
class WAIOARepManager
{
    ...
    ros::ServiceClient ser_cli_gazebo_set_model_configuration;
    ros::ServiceClient ser_cli_gazebo_get_link_properties;
    ros::ServiceClient ser_cli_gazebo_set_link_properties;
    ros::ServiceClient ser_cli_gazebo_set_world_physics_properties;
...
public:
    ...
    void SetRepStateJointViaService(std::string s_rep_name,std::string s_rep_link_name="link_base",std::string s_rep_joint_name="joint_link_base",float f_joint_position=0.0f);
    void GetRepLinkPropertiesViaService(std::string s_rep_link_name,tf::Vector3* vc3_position,tf::Quaternion* qua_orientation,bool* b_gravity_mode,float* f_mass,tf::Vector3* vc3_inertia);
    void SetRepLinkPropertiesViaService(std::string s_rep_link_name,tf::Vector3 vc3_position,tf::Quaternion qua_orientation,bool b_gravity_mode,float f_mass,tf::Vector3 vc3_inertia);
    void SetWorldPhysicsPropertiesViaService(tf::Vector3 vc3_gravity,
                                      double d_time_step=0.005,
                                      double d_max_update_rate=200.0,
                                      bool b_ode_auto_disable_bodies=false,
                                      int i_ode_sor_pgs_precon_iters=0,
                                      int i_ode_sor_pgs_iters=50,
                                      double d_ode_sor_pgs_w=1.3,
                                      double d_ode_sor_pgs_rms_error_tol=0.0,
                                      double d_ode_contact_surface_layer=0.001,
                                      double d_ode_contact_max_correcting_vel=100.0,
                                      double d_ode_cfm=0.0,
                                      double d_ode_erp=0.2,
                                      int i_ode_max_contacts=20);
