...
class WAIOASessionManager
{
    ...
    std::string m_s_session_filename;
    ...
    std::string m_s_path_resources_sessions; // Path where all SESSIONS are collected!
    std::string m_s_session_name_prev;
    std::string m_s_path_resources_scene_preview;
...
public:
    ...
    void Initialize(ros::NodeHandle* hdl_node,float f_node_sample_frequency);
    ...
    void UpdateSessionSlidesConfigFromExport();
    void LoadSession();
    ...
    void UpdateSessionFilename(std::string s_session_filename);
    std::string GetSessionNameFromFilename(std::string s_session_filepath);
    void SetSessionSceneCountCurrent(bool b_increment_decrement,int i_scene_select=-1);
    void SchedulerStart();
    void SchedulerStop();
    ...
    void PrepareScaffoldingMode(std::string s_rep_name,tf::Vector3 vc3_rep_position);
    ...
    std::string GetPathResourcesSessions();
    std::string GetPathResourcesScenePreview();
