...
class GazeboRosForce : public ModelPlugin
{
  ...
  private: void UpdateObjectState(const geometry_msgs::Pose::ConstPtr& _msg);

  /// \brief The custom callback queue thread function.
  private: void QueueThread();
  private: ros::Time tim_iteration_last;
  private: bool m_b_tf_only;
  ...
  private: ros::Subscriber sub_state_;
  ...
  private: std::string topic_name_state_;
  ...
  private: geometry_msgs::Pose pose_msg_;

  // Pointer to the update event connection
  private: event::ConnectionPtr update_connection_;

  // Publish transform
  private:  tf::TransformBroadcaster brc_transforms;
            std::string str_world_name;
