...
void GazeboRosP3D::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  this->last_time_.sec=ros::Time::now().sec; // CHANGED DUE TO sim_time cpu-load/BW problem!
  this->last_time_.nsec=ros::Time::now().nsec;
...
void GazeboRosP3D::UpdateChild()
{
  ...
  cur_time.sec=ros::Time::now().sec; // CHANGED DUE TO sim_time cpu-load/BW problem!
  cur_time.nsec=ros::Time::now().nsec;
