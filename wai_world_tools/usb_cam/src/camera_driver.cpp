...
bool AbstractV4LUSBCam::init_decoder()
{
    ...
    if(!full_ffmpeg_log)
    {
        ...
        printf("Setting usb_cam log level to quiet!");
    }
    else
    {
...
bool AbstractV4LUSBCam::decode_ffmpeg(const void *src, int len, camera_image_t *dest)
{
    ...
    if(avcodec_send_packet(avcodec_context, avpkt) < 0 && full_ffmpeg_log)
    {
    ...
    if (!got_picture && full_ffmpeg_log)
    {
        printf("FFMPEG: buffer empty: expected picture data\n");
        return;
    }
#endif
    if (avcodec_receive_frame(avcodec_context, avframe_camera) < 0 && full_ffmpeg_log)
    {
    ...
    if (pic_size != avframe_camera_size && full_ffmpeg_log)
    {
...
#endif
    if (size != avframe_rgb_size && full_ffmpeg_log)
    {
