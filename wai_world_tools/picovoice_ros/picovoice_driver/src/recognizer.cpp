...
void Recognizer::recognizeThread()
{
  ...
  // PicoLLM - No audio device I/O needed:
  if(record_settings.audio_device_access==0)
  {
      try
      {
          int16_t* dummy;
          recognizeProcess(dummy);
      }
      catch (const std::exception& e)
      {
        throw std::runtime_error("recognizeProcessPrompt failed: " + std::string(e.what()));
      }
      is_recognizing_.store(false);
      return;
  }
  else if(record_settings.audio_device_access==-1) // Cheetah - Audio recorder necessary:
  {
      pv_recorder_t* recorder = NULL;
      pv_recorder_status_t recorder_status = pv_recorder_init(
                  record_settings.frame_length_,
                  -1,
                  100,
                  &recorder);
  ...
  else if(record_settings.audio_device_access==1) // Orca - Audio speaker (out device) necessary:
  {
      // Currently directly writing to WAV file:
      try
      {
          std::cout << "Reached TRY recognizeProcess" << std::endl;
          int16_t* dummy;
          recognizeProcess(dummy);
      }
      catch (const std::exception& e)
      {
        throw std::runtime_error("recognizeProcessSynthetization failed: " + std::string(e.what()));
      }
      is_recognizing_.store(false);
      return;
  }
  ...
  {
      throw std::runtime_error("Wrong audio device access mode!");
  }
}
