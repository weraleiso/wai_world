...
{
    constexpr double EXECUTE_PERIOD = 0.05; // 0.01
    ...
    public:
        ...
        const typename RecognizerDataType::Parameters& parameters) : action_server_(action_name,boost::bind(&RecognizerNode::executeCallback,this,_1),false),parameters_(parameters)
        {
            ros::NodeHandle local_nh("~");
            auto record_directory = local_nh.param("record_directory", defaultRecordDirectory(name));
            record_timeout_ = local_nh.param("record_timeout",30.0);
            if (!local_nh.getParam("access_key", parameters_.access_key_))
            {
                throw std::runtime_error("~access_key not specified");
            }
            recognizer_.initialize(record_directory, record_timeout_);
            dynamic_reconfigure_server_.registerVariable<double>("sensitivity", &parameters_.sensitivity_,"Recognizer sensitivity", 0., 1.);
            dynamic_reconfigure_server_.publishServicesTopics();
            action_server_.start();
            ROS_INFO("RecognizerNode(name=%s, action_name=%s, record_directory=%s, record_timeout=%.2f) initialized with parameters %s",
                                                                                                        name.c_str(),
                                                                                                        action_name.c_str(),
    ...
    private:
        ...
        void executeCallback(const typename ActionType::_action_goal_type::_goal_type::ConstPtr& goal)
        {
            ...
            tim_recognize_start_=ros::Time::now();
            recognizer_.recognize();
            try
            {
                while(ros::ok() && recognizer_.isRecognizing())
                {
                    //if((ros::Time::now()-tim_recognize_start_).toSec()<GetRecognizerTimeout())
                    //{
                    ...
                    {
                        std::string msg = "Recognize error: Timout";
                        ROS_ERROR("%s", msg.c_str());
                        action_server_.setAborted(action_result, msg);
                        return;
                    }
