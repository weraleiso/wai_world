...
### 1.0 Scripted Installation (bash-Script)
Navigate to the scripts-Folder inside the **wai_world\wai_world_bringup** package and run the **wai_world_install.bash** script file:
```
cd ~\catkin_ws\src\wai_world\wai_world_bringup\scripts
./wai_world_install.bash
```
...
```
sudo apt-get install ssh* pssh net-tools meld qtcreator qtmultimedia5-* xmlstarlet cutycapt python3-pip
```
Re-Install/Update LibreOffice to prevent conflicts with outdated packages when using ooo-dev-tools:
```
...
```
Install additional LibreOffce (OpenOffice) ODP Development tools:
```
...
Currently, the (voice) prompt interactions in OA are based on the llama-3.2-3b-instruct-505.pllm chat model.
...
- Once you are registered, provide your own access key in the *wai_oa_intel.cpp* file (line 59) in the **wai_oa_control_center** package.
- Additionally, download the *llama-3.2-3b-instruct-505.pllm* file via your account, and move it to the *.../resources/picovoice* folder in the **wai_oa_gazebo** package.
- Finally, check for proper versions of the [**cheetah**](https://github.com/Picovoice/cheetah), [**orca**](https://github.com/Picovoice/orca), [**picollm**](https://github.com/Picovoice/picollm), [**porcupine**](https://github.com/Picovoice/porcupine), [**pvrecorder**](https://github.com/Picovoice/pvrecorder), [**pvspeaker**](https://github.com/Picovoice/pvspeaker), and [**rhino**](https://github.com/Picovoice/rhino) engines (The current version of the modules is V3.0, whereas Rhino is at V4.0). At some point you may want to upgrade to more recent versions of the engine, however, keep in mind that you eventually have to adapt the Picovoice interfaces in the *wai_picovice.h* file inside the **wai_oa_control_center** package.
...
- [**weraleiso/wai_world/wai_oa**](https://github.com/weraleiso/wai_world/tree/master/wai_oa) Open Auditorium (OA) is a sub-workspace of the wai_world workspace, implementing a human-centric eXtended Reality (XR) Blended Learning (BL) tool. In addition, this workspace holds INTEL (oversight mechanism) and MARVIN (helper display drone) as assistive AIS.
...
## References (MLA):
- Isop, Werner Alexander. [**"Open Auditorium-A Human-Centric Extended Reality Blended Learning Tool."**](https://www.frontiersin.org/journals/artificial-intelligence/articles/10.3389/frai.2026.1698599/full) Frontiers in Artificial Intelligence 9 (2026): 1698599.
- Isop, Werner Alexander. [**"A conceptual ethical framework to preserve natural human presence in the use of AI systems in education."**](https://www.frontiersin.org/journals/artificial-intelligence/articles/10.3389/frai.2024.1377938/full) Frontiers in artificial intelligence 7 (2025): 1377938.
- Isop, Werner Alexander. [**"Acceptance Evaluation of a Digital Extended Reality Teaching and Learning Tool to Support Constructivist-Oriented Teaching Methods in Engineering Education."**](https://didacticum.phst.at/index.php/didacticum/article/view/148) Didacticum, 7(1), pp.47-67 (2024).
