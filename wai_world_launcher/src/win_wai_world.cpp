    ...
    ,m_hdl_node()
{
    ...
    for (const QHostAddress &address: QNetworkInterface::allAddresses())
    {
        if (address.protocol() == QAbstractSocket::IPv4Protocol && address != localhost)
        {
             ...
             win_oa->SetWAIOAPresenterListenerID(GetLastOctetOfIPAsInt(ui->lne_wai_world_infra_ip_local->text().toStdString())-1);
        }
    }
    if(ui->lne_wai_world_infra_ip_local->text()=="")
    {
        ...
        win_oa->SetWAIOAPresenterListenerID(0);
    }
}
...
int win_wai_world::GetLastOctetOfIPAsInt(std::string s_ip_adress)
{
    std::size_t siz_dot = s_ip_adress.find_last_of(".");
    int i_last_octet=std::stoi(s_ip_adress.substr(siz_dot+1));
    return i_last_octet;
}
...
void win_wai_world::on_btn_wai_world_launcher_clicked()
{
    ...
    if(win_oa->GetWAIOAPresenterMode()==Qt::Checked)//ui->chk_wai_world_infra_pre_or_aud->text()=="Presenter")
    {
        s_syscmd="export WAI_OA_PRESENTER_ID=\""+
                std::to_string(win_oa->GetWAIOAPresenterListenerID())+
                ...
                //+" oa_session_filename:="+win_oa->GetSessionSelectedFilename();
    ...
    // Confirm proper shutown of WAI World process
    QMessageBox mgs_wai_world_confirmation;
...
void win_wai_world::on_btn_wai_world_shortcut_oa_settings_clicked()
{
    win_oa->show();
}
